...
class CustomRNADataset(Dataset):
    def __init__(self, root_dir, pair_indices, df_data, augmentation=False):
    ...
    def __getitem__(self, idx):
        try:
            ...
            if not all(os.path.exists(p) for p in [mol_path, pdb_path]): 
                return None
            ...
            rna_seq_padded = pad_sequence(str(row['seq']), MAX_SEQ_LEN, RNA_SEQ_CHAR_TO_INT)
            smiles_padded = pad_sequence(str(row['smiles']), MAX_SMILES_LEN, SMILES_CHAR_TO_INT)
...
def loop(model, dataloader, optimizer, criterion, device, is_train=True):
    ...
    if len(dataloader) == 0:
         return {"loss": 0, "acc": 0, "f1": 0, "rocauc": 0, "pre": 0, "rec": 0}
...
def get_valid_indices(data_dir, all_indices):
    valid_indices = []
    print("Checking processed files integrity...")
    for idx in all_indices:
        pair_folder = os.path.join(data_dir, f'pair_{idx:05d}')
        mol_p = os.path.join(pair_folder, 'molecule_graph.npz')
        pdb_p = os.path.join(pair_folder, 'unrelaxed_model.pdb')
        
        if os.path.exists(mol_p) and os.path.exists(pdb_p):
            valid_indices.append(idx)
        else:
            print(f"Warning: Missing files for pair_{idx:05d}")
...
def main(config):
    ...
    try:
        df = pd.read_csv(config['data_file'], sep=r'\s+', header=None, names=['smiles', 'seq', 'label'], engine='python')
    except Exception as e:
    ...
    print(f"Total entries: {len(df)}. Valid processed entries: {len(valid_indices)}")
    ...
    if len(valid_indices) < 5:
        ...
        print("[WARNING] Running in DEMO MODE: Using the same data for Train/Val/Test.")
        train_idx = valid_indices
        val_idx = valid_indices
        test_idx = valid_indices
        ...
        try:
            ...
            print(f"\n[ERROR] Train_test_split failed (likely due to small data size): {e}")
            print("Fallback: Using all data for training (no validation split).")
            train_idx = valid_indices
            val_idx = valid_indices
            test_idx = valid_indices
            train_labels = labels_valid

    train_set = CustomRNADataset(config['data_dir'], train_idx, df, augmentation=True)
    val_set = CustomRNADataset(config['data_dir'], val_idx, df, augmentation=False)
    test_set = CustomRNADataset(config['data_dir'], test_idx, df, augmentation=False)

    train_loader = DataLoader(train_set, batch_size=config['batch_size'], shuffle=True, collate_fn=filter_none_collate_quad, num_workers=0)
    val_loader = DataLoader(val_set, batch_size=config['batch_size'], shuffle=False, collate_fn=filter_none_collate_quad, num_workers=0)
    test_loader = DataLoader(test_set, batch_size=config['batch_size'], shuffle=False, collate_fn=filter_none_collate_quad, num_workers=0)
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        print(f"Epoch {epoch}/{config['epochs']} | Loss: {train_met['loss']:.4f} | Train Acc: {train_met['acc']:.2f} | Val F1: {val_met['f1']:.4f}")

    print("\nEvaluating on Test Set...")
    try:
        model.load_state_dict(torch.load(save_path))
    except:
        print("Warning: Could not load best model, using last epoch.")
