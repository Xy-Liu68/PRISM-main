...
SMRTNET_DATA_FILE = './PRISM-main/data/benchmark_all.txt'
...
OUTPUT_DIR = './PRISM-main/3D_processed_output/all_benchmark_processed_output'
...
def main():
    print("========================================================")
    print("Starting SMRTnet BENCHMARK data processing...")
    print(f"Reading from: {SMRTNET_DATA_FILE}")
    ...
    try:
        df = pd.read_csv(
            ...
            sep='\t', 
            header=None, 
            names=['id', 'smiles', 'rna_sequence', 'dot_bracket', 'label']
        )

    except Exception as e:
        print(f"[FATAL ERROR] Failed to read SMRTnet Benchmark data file. Error: {e}")
        return
        
    print(f"Found {len(df)} benchmark pairs to process.")
    ...
    for index, row in progress_bar:
        ...
        progress_bar.set_description(f"Processing Benchmark {pair_id}")
    ...
    print(f"All benchmark outputs are saved in: {OUTPUT_DIR}")
    print("========================================")
